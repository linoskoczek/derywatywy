...
pd_list = [[h_pl.syllables(c[0]), h_pl.syllables(
    c[1]), c[0], c[1]] for c in data.values]
pd_list = [[p[0][-1], p[1][-1], p[2], p[3]]
           for p in pd_list if p[0] != '' and p[0] != []]
...
for neigh in algs:
    ...
    print("Score for", type(neigh).__name__,
          accuracy_score(pred['fs'], test['fs']))
